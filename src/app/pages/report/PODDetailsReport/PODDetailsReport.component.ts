...
@Component({
  selector: "app-PODDetailsReport",
  templateUrl: "PODDetailsReport.component.html",
})
export class PODDetailsReportComponent implements OnInit {
  entries: number = 10;
  selected: any[] = [];
  temp = [];
  activeRow: any;
  ...
  _ColNameList = ["PODNO", "Location","CourierName","SenderemailID","SenderPODEntryDatetime","CrownAcknowledgedatetime", "DetailInwarddate", "SenderInvoicecount","ReceivedInvoicecount","DetailInwardInvoicecount","Status"];
  ...
  filterTable($event) {
    ...
    this._FilteredList = this._StatusList.filter(function (d) {
      console.log(d);
      for (var key in d) {
        if (key == "PODNO" || key == "CourierName" || key == "Status" ) {
  ...
  getStatusList() {  
    ...
    const apiUrl = this._global.baseAPIUrl + 'Inward/GetStatusReport';          
  ...
  downloadFile() { 
    ...
    dwldLink.setAttribute("download",  "POD Report" + ".csv"); 
