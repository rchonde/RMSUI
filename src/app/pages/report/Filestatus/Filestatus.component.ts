...
export class FilestatusComponent implements OnInit {
  ...
  _ColNameList = ["FileNo","FileUploadDate","PageCount","FileUploadStatusremark","Remark"];
  ...
  filterTable($event) {
    ...
    this._FilteredList = this._StatusList.filter(function (d) {
      console.log(d);
      for (var key in d) {
        if (key == "FileNo" || key == "FULLFIRMNAME" || key == "SACKCODE" ) {
  ...
  getStatusList() {  
    ...
    const apiUrl = this._global.baseAPIUrl + 'Inward/GetFileStatusReport';          
